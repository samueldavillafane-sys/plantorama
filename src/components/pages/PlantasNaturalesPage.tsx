...
export default function PlantasNaturalesPage() {
  const [plants, setPlants] = useState<PlantasNaturales[]>([]);
  ...
  const loadPlants = async () => {
    try {
      const result = await BaseCrudService.getAll<PlantasNaturales>('naturalplants');
      ...
      <section className="w-full max-w-[100rem] mx-auto px-6 py-16">
        ...
        >
          ...
          <p className="font-paragraph text-xl text-foreground max-w-3xl mx-auto">
            Descubre la flora nativa que embellece nuestros espacios naturales en Barranquilla
          </p>
        </motion.div>
          ...
          {isLoading ? null : plants.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {plants.map((plant, index) => (
                ...
                >
                  <Link to={`/plantas-naturales/${plant._id}`}>
                    <motion.div
            ...
            <div className="text-center py-16">
              <p className="font-paragraph text-xl text-foreground opacity-60">
                No hay plantas naturales disponibles en este momento.
              </p>
            </div>
          )}
        </div>
